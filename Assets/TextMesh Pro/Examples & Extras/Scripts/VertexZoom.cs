        ...
        IEnumerator AnimateVertexColors()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < textInfo.meshInfo.Length; i++)
                {
                    ...
                    // Convert Vector4[] to Vector2[] for compatibility with mesh.uv
                    Vector2[] uv2D = textInfo.meshInfo[i].uvs0.Select(uv => new Vector2(uv.x, uv.y)).ToArray();
                    textInfo.meshInfo[i].mesh.uv = uv2D;
